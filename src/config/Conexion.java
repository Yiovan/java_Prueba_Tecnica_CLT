...
public class Conexion {
    ...
    private static String requireEnv(String key) {
        ...
        if (v == null || v.isBlank()) v = System.getenv(key);
        if (v == null || v.isBlank()) {
            throw new IllegalStateException("Variable de entorno requerida no configurada: " + key + " (copia .env.example a .env)");
        }
        return v;
    }
    ...
    public static Connection conectar() {
        try {
            ...
            System.out.println("¡Conexión a la base de datos exitosa! " + DB_HOST + ":" + DB_PORT + "/" + DB_NAME + " (user=" + USER + ")");
            return conexion;
        } catch (SQLException e) {
            System.err.println("Error al conectar: " + e.getMessage() + " host=" + DB_HOST + " db=" + DB_NAME + " user=" + USER);
            return null;
        }
    }
